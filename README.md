...
`open-course-player` 是一个面向本地文件目录和素材目录的开源桌面播放器。

第一目标是 Windows 桌面端优先稳定播放 `.ts` 文件；长期目标是提供一个好用的本地文件播放器：左侧目录树，右侧播放区，支持主流视频格式、播放进度、字幕、快捷键和开源合规打包。
...
当前实现优先面向 Windows 桌面端。

Linux 和 macOS 属于桌面兼容目标，需要在对应系统完成打包和播放验证后，才能在发布说明中标记为已支持。iOS 虽然属于 Tauri 2 可构建目标，但它不是桌面文件夹播放器模型：移动端需要使用文件导入/文件选择、应用沙盒存储和移动端播放器能力，不能依赖任意目录扫描、打开文件所在位置或 mpv 子进程。
